...
const FALLBACK_ETFS: ETFHolding[] = [
  ...
  { name: "ARK 21Shares Bitcoin ETF", ticker: "ARKB", held: 48_000, aum: 4.73e9, flows30d: 210e6 },
...
async function fetchYahooChart(ticker: string): Promise<ChartResult | null> {
  try {
    ...
    // Get 30-day-ago close from first data point
    const closes: (number | null)[] =
      result.indicators?.quote?.[0]?.close ?? [];
    const firstClose =
      closes.find((c: number | null) => c != null && c > 0) ?? price;

    return { price, price30dAgo: firstClose as number, name };
  } catch {
    return null;
  }
}
...
async function fetchBtcPrice(): Promise<{
  ...
  try {
    ...
    return {
      current: prices[prices.length - 1][1],
      price30dAgo: prices[0][1],
    };
  } catch {
    return null;
  }
...
export async function GET() {
  ...
  try {
    ...
    for (let i = 0; i < ETF_TICKERS.length; i++) {
      ...
      const btcPriceRatio = btcData.current / btcData.price30dAgo;
      const expectedAumFromPriceOnly = aum30dAgo * btcPriceRatio;
      const flows30d = Math.round(aum - expectedAumFromPriceOnly);
