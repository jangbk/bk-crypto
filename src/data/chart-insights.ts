import type { ChartPriceLine } from "@/components/dashboard/LightweightChartWrapper";
...
export const CHART_INSIGHTS: Record<string, ChartInsightConfig> = {
  "btc-risk-signal": {
    metric: "RSI (14일)",
    unit: "",
    bands: [
...
export const DUAL_CHART_CONFIG: Record<string, { secondaryApi: string; label: string; color: string }> = {
...
export const CHART_BAND_LINES: Record<string, { primary?: ChartPriceLine[]; secondary?: ChartPriceLine[] }> = {
  "btc-risk-signal": {
    primary: [
...
export interface ChartAboutContent {
...
export const CHART_ABOUT: Record<string, ChartAboutContent> = {
  "total-market-cap": {
...
이 차트에는 로그 회귀 트렌드라인(Logarithmic Regression Trendline)이 포함되어 있습니다. ln(시가총액) = a × ln(일수) + b 형태의 멱법칙(Power Law) 모델로, 2013년부터의 전체 역사 데이터를 기반으로 장기 공정가치(Fair Value)를 추정합니다. 빨간색 실선이 공정가치이며, 녹색 대시선은 ±2σ(표준편차) 밴드입니다. 가격이 상한 밴드에 닿으면 역사적 과대평가 구간, 하한 밴드에 닿으면 극단적 저평가 구간으로 해석됩니다. 차트를 좌우로 스크롤하면 미래 3년간의 추세 예측도 확인할 수 있습니다.`,
    assetRanking: {
...
이 차트에는 2010년부터의 전체 역사 데이터를 기반으로 한 로그 회귀 트렌드라인이 포함되어 있습니다. 빨간색 Fair Value 선은 비트코인의 장기 공정가치를 나타내며, 녹색 대시선(±2σ 밴드)은 역사적 과대/과소 평가 영역을 표시합니다. 비트코인은 매 사이클마다 상한 밴드를 터치한 후 하락하고, 하한 밴드 부근에서 바닥을 형성하는 패턴을 반복해왔습니다. 차트를 오른쪽으로 스크롤하면 미래 3년간의 추세 예측도 확인할 수 있습니다.`,
    assetRanking: {
...
이 차트에는 2015년부터의 전체 역사 데이터를 기반으로 한 로그 회귀 트렌드라인이 포함되어 있습니다. 빨간색 Fair Value 선은 이더리움의 장기 공정가치이며, 녹색 ±2σ 밴드는 과대/과소 평가 영역입니다. 이더리움은 비트코인 대비 역사가 짧아 변동성이 더 크며, 밴드 폭이 넓게 형성됩니다.`,
    assetRanking: {
...
차트에 표시된 로그 회귀 트렌드라인(Fair Value)은 2014년부터의 알트코인 시가총액 데이터에 대수 회귀(ln(시총) = a × ln(일수) + b)를 적용한 것입니다. ±2σ 밴드는 Fair Value 대비 과대평가(상단) 및 과소평가(하단) 구간을 나타냅니다. 알트코인 시장은 BTC보다 변동성이 크기 때문에 밴드 폭이 더 넓게 나타나며, 이는 알트 시즌의 극단적 과열과 알트 윈터의 극심한 하락을 반영합니다.`,
  },
...
차트에 표시된 로그 회귀 트렌드라인(Fair Value)은 2017년부터의 스테이블코인 총 시가총액에 대수 회귀를 적용한 것입니다. 스테이블코인은 가격이 $1에 고정되어 있으므로 시가총액 증가는 순수한 발행량(채택) 확대를 의미합니다. ±2σ 밴드는 장기 성장 추세 대비 과도한 발행(상단, 과열기 레버리지 수요) 또는 발행 위축(하단, 자금 유출기)을 나타냅니다.`,
    assetRanking: {
...
export const CHART_ABOUT_MACRO_TRADFI: Record<string, ChartAboutContent> = {
  // ─── Macro Charts ────────────────────────────────────────────────────
  "us-real-gdp": {
