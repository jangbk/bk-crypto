...
export function CapitulationIndicator({ data }: { data: CapitulationData }) {
  ...
  const statusBorder: Record<string, string> = {
    매수: "border-positive/30 text-positive",
    매도: "border-negative/30 text-negative",
    중립: "border-warning/30 text-warning",
  };

  const puellZoneColors: Record<string, string> = {
    undervalued: "bg-positive/15 text-positive",
    neutral: "bg-warning/15 text-warning",
    overvalued: "bg-negative/15 text-negative",
  };
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5">
      ...
      {/* Capitulation Guide */}
      {showGuide && (
        <div className="mb-5 rounded-lg bg-negative/5 border border-negative/15 p-4 space-y-3 text-sm text-foreground/80">
          <div>
            <h4 className="font-semibold text-negative mb-1">채굴자 항복(Miner Capitulation)이란?</h4>
          ...
          <div>
            <h4 className="font-semibold text-negative mb-1">왜 중요한가?</h4>
          ...
          <div>
            <h4 className="font-semibold text-negative mb-1">역사적 사례와 시사점</h4>
            <ul className="list-disc list-inside space-y-1 text-foreground/70">
              ...
              <li><strong className="text-foreground">핵심</strong>: 채굴자 항복은 <span className="text-positive">역발상 매수 기회</span>로, 공포가 극에 달한 시점이 장기 투자자에게 최적의 진입 시점이었습니다</li>
            </ul>
          </div>
          <div>
            <h4 className="font-semibold text-negative mb-1">주의사항</h4>
        ...
        <div className="space-y-3">
          ...
          <div className={`rounded-lg border p-4 ${statusBorder[data.hashRibbon.status]}`}>
            ...
            {data.hashRibbon.status === "매수" && (
              <div className="mt-3 flex items-center gap-2 bg-positive/8 rounded-lg p-3 border border-positive/20">
                <Target className="w-5 h-5 text-positive shrink-0" />
                <div>
                  <p className="text-sm font-bold text-positive">BUY SIGNAL ACTIVE</p>
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              <div className="flex h-3 rounded-full overflow-hidden">
                <div className="w-1/4 bg-positive/60" />
                <div className="w-1/2 bg-warning/60" />
                <div className="w-1/4 bg-negative/60" />
              </div>
