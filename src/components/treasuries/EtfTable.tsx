      ...
      <div className="rounded-lg border border-border bg-muted/30 p-3 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-1.5 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5">
            ...
            <span><strong className="text-positive">+금액 (초록)</strong> = 순유입 — ETF로 자금 유입, 매수 수요 증가 → 가격 상승 압력</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 rounded-sm bg-negative/70" />
            <span><strong className="text-negative">-금액 (빨간)</strong> = 순유출 — ETF에서 환매, 매도 우위 → 가격 하락 압력</span>
          </div>
        </div>
      </div>
          ...
          <tbody>
            {etfs.map((row) => (
              <tr key={row.ticker} className="border-b border-border hover:bg-muted/30 transition-colors">
                ...
                <td className={`px-4 py-3 text-right font-mono ${row.flows7d >= 0 ? "text-positive" : "text-negative"}`}>
                  {row.flows7d >= 0 ? "+" : ""}{formatCurrency(Math.abs(row.flows7d))}
                </td>
                <td className={`px-4 py-3 text-right font-mono ${row.flows30d >= 0 ? "text-positive" : "text-negative"}`}>
            ...
            <tr className="bg-muted/30 font-semibold">
              ...
              <td className={`px-4 py-3 text-right font-mono ${etfs.reduce((s, e) => s + e.flows7d, 0) >= 0 ? "text-positive" : "text-negative"}`}>
                {etfs.reduce((s, e) => s + e.flows7d, 0) >= 0 ? "+" : ""}
                {formatCurrency(Math.abs(etfs.reduce((s, e) => s + e.flows7d, 0)))}
              </td>
              <td className={`px-4 py-3 text-right font-mono ${etfs.reduce((s, e) => s + e.flows30d, 0) >= 0 ? "text-positive" : "text-negative"}`}>
